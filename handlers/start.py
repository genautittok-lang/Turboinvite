...
async def cmd_start(message: Message):
    ...
    if not user:
        user = User(
            ...
            language='uk', currency='UAH', balance=0.0, invited=0, level='Newbie'
        )
        # Якщо є реферальний payload, збільшуємо лічильник у реферера
        if payload:
            try:
                ...
                if ref_user:
                    ...
                    # Можна додати транзакцію бонусу, якщо потрібно
                except Exception:
                    pass
        db.add(user)
        db.commit()
